...
(deftest repo-key-test
  (let [{{:keys [org0]} :ents db :db} (mock-gen/gen-ents {:org [[1]]})]
...
(deftest repo-patch-test
  (let [{{:keys [org0]} :ents db :db} (mock-gen/gen-ents {:org [[:org0]]})]
    (is (match? {:repo/name+org ["repo" (:db/id org0)]}
                (repo/repo-patch db {:path-params {:org (:org/name org0)
                                                   :repo "repo"}}))))
  (let [{{:keys [org0 repo0]} :ents db :db} (mock-gen/gen-ents {:repo [[1 {:spec-gen {:repo/attrs {:some-attr "value"}}}]]})]
...
(deftest repo-list-test
  (let [{{:keys [org0 repo0 repo1]} :ents db :db} (mock-gen/gen-ents {:repo [[2]]})]
...
(deftest branch-body-test
  (let [{{:keys [repo0 branch0]} :ents} (mock-gen/gen-ents {:branch [[1 {:spec-gen {:ref/ref "refs/heads/my-branch"}}]]})]
